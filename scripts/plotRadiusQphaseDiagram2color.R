...
# splitting the heat color into only two values at arbitrary
# border (0.1 as used in Klemm et. al 2003 Phys Rev E 67:026120).
...
# Usage:
#
# Rscript plotRadiusQphaseDiagram2color.R data.csv outputfilenameprefix
#
#
# E.g. Rscript plotRadiusQphaseDiagram2color.R results.csv phase_diagram
#
...
cutvalue <- 0.1 # cutoff value of order parameter (max region size),
                #  see header comment
...
if (length(commandArgs(trailingOnly=TRUE)) != 2) {
    cat("Usage: Rscript plotRadiusQphaseDiagram2color.R results.csv outputprefix\n")
    quit(save='no')
}
...
Dst <- transform(Dst, value.mean = cut(value.mean, 
                                       breaks=c(0, cutvalue, 1),
...
p <- p + scale_fill_brewer(labels=c("multicultural", "monocultural"),
                           palette = "Set3",
                           name="")
#p <- p + scale_fill_manual(labels=c("multicultural", "monocultural"),
#                             values=c('gray', 'white'))
